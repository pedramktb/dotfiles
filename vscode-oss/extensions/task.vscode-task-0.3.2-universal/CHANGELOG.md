...
- Fixed some bugs with the tree view not displaying tasks correctly (#59, [#79](https://github.com/go-task/vscode-task/issues/79),
  [#90](https://github.com/go-task/vscode-task/issues/90) by @MaxCheetham).
...
- Allow commands to be run from last active terminal instead of the output panel
  (#12, [#43](https://github.com/go-task/vscode-task/issues/43) by @pd93).
  - Configurable via `task.outputTo` setting (values: `output` (default) or
    `terminal`).
- This extension is now also published on the
  [Open VSX Registry](https://open-vsx.org/extension/task/vscode-task) (#26, [#46](https://github.com/go-task/vscode-task/issues/46)
  by @pd93).
...
- Fixed some installations (e.g. Brew) not detecting the Task version correctly
  (#13, [#14](https://github.com/go-task/vscode-task/issues/14) by @pd93).
